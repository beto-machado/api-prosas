...
module Api
  module V1
    class PropostasController < ApplicationController
      include RenderMeta
      before_action :set_proposta, only: %i[show update destroy]

      # GET /api/v1/propostas
      def index
        @propostas = if search.present?
          Proposta.search(search).sorted.page(params[:page])
        else
          Proposta.sorted.page(params[:page])
        end
